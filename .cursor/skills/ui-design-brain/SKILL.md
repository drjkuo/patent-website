...
description: >-
  ...
  Use when building or refining web interfaces, landing pages, navigation, forms, or any UI in this repo or elsewhere.
  In the patent-website project, prefer extending static HTML/CSS/JS (index.html, css/styles.css, js/main.js) unless the user specifies another stack.
---

# UI Design Brain

Upstream project: [carmahhawwari/ui-design-brain](https://github.com/carmahhawwari/ui-design-brain) (MIT — see [LICENSE.txt](LICENSE.txt)). Component patterns informed by [component.gallery](https://component.gallery).
...
## This repository (patent-website)

When the task is limited to **this repository**:

- **Stack:** static **HTML + CSS + JavaScript** — `index.html`, `css/styles.css`, `js/main.js`. Keep GitHub Pages–friendly output (no required build step) unless the user asks otherwise.
- **Apply** the same principles, component models, **8 px spacing rhythm**, accessibility expectations, and anti-patterns from this skill. Map “Tailwind-style” spacing mentally to `rem` / `clamp()` and consistent tokens (this site already uses CSS variables in `:root`).
- **Typography:** continue the existing **DM Sans** + **Fraunces** pairing and heading hierarchy unless a redesign explicitly changes it.
- **Do not** default to **React + Tailwind** for edits here; only use that stack if the user clearly requests it.
...
**patent-website (this repo) — use instead of the generic stack below:**

```
Stack:        Semantic HTML5, plain CSS (tokens in :root), minimal JS
Spacing:      8px rhythm via rem/clamp; align with existing section/card patterns
Colors:       Existing CSS variables (--bg, --accent, --text, …)
Typography:   DM Sans + Fraunces (already linked); maintain h1 → h2 → h3 order
States:       :hover, :focus-visible, active/disabled as needed; keep skip-link
Responsive:   Mobile-first; match breakpoints used in css/styles.css
Accessibility: Semantic regions, aria-* on toggles, keyboard-navigable nav
```

**Other projects (default from upstream):**
